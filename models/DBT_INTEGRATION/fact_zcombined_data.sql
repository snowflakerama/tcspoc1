{{ config(materialized='table',schema='dbt_integration',alias='tbl_fact_all_ret_data') }}

with D as (
    
    select * from {{ source('dbt_dev','dim_customer') }} 

),

E as  (

    select * from {{ source('dbt_dev','dim_product')}}
),

B as (

    select * from {{ source('dbt_dev','fact_orders')}}
),
...
C as (
    ...
    select * from {{ source('dbt_dev','dim_date')}}
),

P as (

    select * from {{ source('dbt_dev','dim_people')}}
),
